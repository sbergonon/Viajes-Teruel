...
const FUNCTION_TIMEOUT = 9000; // 9 segundos
...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
    ...
    try {
        ...
        const tripIdeas = JSON.parse(jsonText);
        ...
        return {
            ...
            body: JSON.stringify(tripIdeas),
        };

    } catch (error) {
        console.error("Error in getTripIdeas function:", error);
        if (error.message === 'Function timed out') {
            return {
                statusCode: 504,
                body: JSON.stringify({ error: "La petición de sugerencias ha tardado demasiado." }),
            };
        }
        return {
            statusCode: 500,
            body: JSON.stringify({ error: "No se pudieron generar las ideas de viaje." }),
        };
    }
};
